...
void updateAccount(struct account accounts[], int n) {
    ...
    printf("Nhap ten moi: ");
    fgets(accounts[pos].fullname, sizeof(accounts[pos].fullname), stdin);
    accounts[pos].fullname[strcspn(accounts[pos].fullname, "\n")] = 0;
    ...
    while (1) {
        ...
        int duplicate = 0;
        for (int i = 0; i < n; i++)
            if (i != pos && strcmp(accounts[i].phone, phone) == 0)
                duplicate = 1;
    ...
    strcpy(accounts[pos].phone, phone);
